...
def get_gmail_service():
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file('credentials.v2.videodrive.json', GMAIL_SCOPES)   #my credentials and scopes
...
def get_drive_service():
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file('credentials.v2.videodrive.json', DRIVE_SCOPES)
            creds = flow.run_local_server(port=0)
